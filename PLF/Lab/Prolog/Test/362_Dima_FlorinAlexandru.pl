...
wroteBy(Author, Result) :-
    authors(AuthorId, Author, _),
    findall(Title, books(_, Title, AuthorId, _, _), Result).
...
editionAfterYear(Year, Result) :-
    findall((BookTitle, AuthorName), (
        editions(_, BookId, _, EditionYear),
        books(BookId, BookTitle, AuthorId, _, _),
...
% To do: Group by Country and count for each group (currently returns a list of (Country, Book title))
countByCountries(Result) :-
    findall((Country, Title), (
        publishers(PublisherId, _, Country),
        books(_, Title, _, PublisherId, _)
    ), Result).
...
splitListEvenOdd(InputList, OutputEvenList, OutputOddList) :-
    findall(X, (member(X, InputList), X mod 2 =:= 0), OutputEvenList),
    findall(X, (member(X, InputList), X mod 2 =:= 1), OutputOddList).
...
% To do
findSublist(L, S, Sublist) :- L, S, Sublist, !.
...
% To do
literalForResolution(C1, C2, Result) :- C1, C2, Result, !.
...
% To do
rezolvent(C1, C2, Result) :- C1, C2, Result, !.
